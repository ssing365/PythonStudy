...
total_days = 0
#월별 날짜수를 리스트로 정의. 1월을 인덱스 1로 사용하기 위해 첫 번째 요소는 0으로 설정
...
윤년은 2월 29일까지 있기 때문이다.
만약 1, 2월을 입력했다면 이 부분은 고려할 필요가 없다.'''
if month >= 3:
    if year % 400 == 0:
...
'''
서기 1년 1월 1일은 월요일이므로 7로 나눈 나머지를 통해 요일을 판단할 수 있다.
나누어 떨어지면 일요일, 1이 남으면 월요일이 된다.'''
if remainder == 0:
    print("일요일입니다.")
if remainder == 1:
    print("월요일입니다.")
